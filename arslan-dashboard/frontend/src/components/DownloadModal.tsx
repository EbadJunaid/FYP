    ...
    const buildDownloadUrl = (): string => {
        ...
        switch (activeFilter.type) {
            ...
            case 'validityTrend':
                ...
                break;
            case 'bucket':
                // validity_bucket like "0-90", "90-365", "365-730", "730+"
                if (activeFilter.value) {
                    params.append('validity_bucket', activeFilter.value);
                }
                break;
            case 'expiringDays':
                // expiring_days like "30", "90"
                if (activeFilter.value) {
                    params.append('expiring_days', activeFilter.value);
                }
                break;
            case 'issuedMonth':
                // issued_month like "2026-01" (YYYY-MM)
                if (activeFilter.value) {
                ...
                break;
            case 'expiredMonth':
                // expired_month like "2026-01" (YYYY-MM) - uses expiring_month/year for certificates expired in that month
                if (activeFilter.value) {
                    const [year, month] = activeFilter.value.split('-').map(Number);
                    if (month > 0 && year > 0) {
