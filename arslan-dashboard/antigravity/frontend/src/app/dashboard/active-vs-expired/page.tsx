...
const certificatesFetcher = async (key: string) => {
    // Parse the key to extract filter and page
    const [, filter, page] = key.split('|');
    const status = filterToStatus[filter as FilterType];
...
export default function ActiveVsExpiredPage() {
    ...
    // SWR for table data with unique key including filter and page
    const swrKey = `certificates|${filter}|${currentPage}`;
