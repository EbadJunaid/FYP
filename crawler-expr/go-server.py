...
GO_BINARY_NAME = "certstream-server-go_1.9.0_macOS_amd64"
...
def is_go_server_running():
    """Checks macOS process tree to see if the Go binary is already active."""
    try:
        # pgrep returns 0 if process matches, 1 if no match
        result = subprocess.run(["pgrep", "-f", GO_BINARY_NAME], capture_output=True, text=True)
...
def terminate_go_server_gracefully():
    """Locally terminates the Go binary via standard SIGTERM (Signal 15)."""
    print(f"🛑 Killing target backend process '{GO_BINARY_NAME}' gracefully...")
    try:
        # This will target the process regardless of who spawned it
        subprocess.run(["pkill", "-15", "-f", GO_BINARY_NAME], capture_output=True)
    except Exception as e:
