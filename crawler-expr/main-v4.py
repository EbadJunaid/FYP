### This is the crawler which removes the raw field and also add the scope field
### This is also doing batch writing to the database which is not recommended because 
### of multi-threaded issues 
...
CONFIG = {
    ...
    'DB_NAME': "new-crawler-1",
    'STATUS_COLLECTION': "domain_status",
    'CERTIFICATES_COLLECTION': "certificates",
    
    # Paths
    'CSV_FILE': os.path.join(BASE_DIR, "./merged-pk-tranco-rapid-x-100.csv"),
    'ZCERT_BINARY': os.path.join(BASE_DIR, "./zcertificate/zcertificate"),
    'LOG_FILE': os.path.join(BASE_DIR,"new-crawler-1.log"),
    'ISSUE_LOG_FILE': os.path.join(BASE_DIR,"new-crawler-1-issues.txt"),
    'NUM_THREADS': 30,
    'SOCKET_TIMEOUT': 10,
    'ZCERT_TIMEOUT': 10,
    'RETRY_ENABLED': False,
    ...
    # Batch writing for certificates
    'CERT_BATCH_SIZE': 50,      # flush once this many certs are buffered
    'CERT_FLUSH_INTERVAL': 5,   # flush every N seconds even if batch isn't full
}
...
cert_buffer = []                    # holds parsed certs waiting to be batch-written
cert_buffer_lock = threading.Lock() # protects cert_buffer across worker threads
...
# -------------------- Batch Certificate Writes --------------------
def write_batch_to_db(batch):
    """Bulk-inserts a list of certificate documents. Duplicate domains are
    expected/ignored; any other write errors are sent to log_issue."""
    if not batch:
        return
    try:
        certs_coll.insert_many(batch, ordered=False)
    except BulkWriteError as bwe:
        other_errors = [e for e in bwe.details.get('writeErrors', []) if e.get('code') != 11000]
        if other_errors:
            log_issue(f"Bulk cert insert had {len(other_errors)} non-duplicate error(s): {other_errors}")
    except Exception as e:
        log_issue(f"Bulk cert insert failed: {e}")

def buffer_certificate(parsed_data):
    """Adds a parsed cert to the shared buffer; flushes if the batch is full."""
    to_flush = None
    with cert_buffer_lock:
        cert_buffer.append(parsed_data)
        if len(cert_buffer) >= CONFIG['CERT_BATCH_SIZE']:
            to_flush = cert_buffer[:]
            cert_buffer.clear()
    if to_flush:
        write_batch_to_db(to_flush)

def cert_flush_thread():
    """Periodically flushes the cert buffer so certs don't sit in memory too
    long when the queue is slow/near the end. Also does a final flush on shutdown."""
    while not shutdown_event.is_set():
        time.sleep(CONFIG['CERT_FLUSH_INTERVAL'])
        to_flush = None
        with cert_buffer_lock:
            if cert_buffer:
                to_flush = cert_buffer[:]
    ...
    # Final flush after shutdown is signaled, so nothing buffered is lost.
    to_flush = None
    with cert_buffer_lock:
        if cert_buffer:
            to_flush = cert_buffer[:]
...
def main():
    ...
    flush_thread = threading.Thread(target=cert_flush_thread, daemon=False)
    flush_thread.start()
    ...
    dashboard_loop()
    flush_thread.join()  # wait for the final buffer flush before exiting
