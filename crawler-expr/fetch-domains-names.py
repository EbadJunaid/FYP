...
DATABASE_NAME    = "tranco-latest-8-lakh"          # ← replace
...
def main() -> None:
    ...
    # Quick sanity-check: how many documents does the collection have?
    try:
    ...
    if total_written == 0:
        print(
            ...
            "Check DATABASE_NAME, COLLECTION_NAME, and DOMAIN_FIELD at the top of this script."
        )
        print("\n⚠  Nothing to do — no documents with a 'domain' field were found.\n")
        return
    ...
    # 4. Ask user about deduplication
    if duplicates:
        try:
            answer = input(
    ...
    print("═" * 60)
    print(f"  📄  Primary CSV   : {Path(OUTPUT_CSV).resolve()}")
    if Path(DEDUP_CSV).exists():
        print(f"  📄  Dedup CSV     : {Path(DEDUP_CSV).resolve()}")
    print()
