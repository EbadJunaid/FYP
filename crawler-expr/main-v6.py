### This file is just the copy of main-v5 and just has command line arguments 
### This is the crawler which removes the raw field and also add the scope field
### This does not have batch writing which is not needed for multi threaded issues 
...
def load_csv_if_empty():
    ...
    if domains:
        try:
            ...
            # BulkWriteError is what insert_many(ordered=False) actually raises
            # when one or more documents hit a duplicate key — the non-duplicate
            # docs are still inserted successfully, so we just swallow this.
...
def main():
    # Parse the arguments first to apply CLI overrides
    parse_arguments()
    
    signal.signal(signal.SIGINT, lambda s, f: shutdown_event.set())
    print("="*60)
    print("      FINAL HYBRID CRAWLER (V3 + V2 LOGS + ALL CLI ARGS)")
    print("="*60)
