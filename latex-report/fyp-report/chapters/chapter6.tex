...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[store] (live-db) at (0,0.4) {Live domain database\\(continuously updated)};

    \node[store] (ct) at (0,-1.6) {Unprocessed CT\\domain observations};

    \node[box] (renew-select) at (-3.8,-3.15) {Match observations with\\known domains};
    \node[box] (new-select) at (3.8,-3.15) {Select previously unseen\\candidate domains};
...
chapter to the concrete artifact that implements it. All scripts reside in
the \texttt{ct-logs-renewal-pipeline/} directory and are coordinated by the
orchestrator described in Section~\ref{sec:eight-stage-orchestrator}.
...
\centering
\small
\begin{tabularx}{\textwidth}{@{}p{3.2cm} p{4.3cm} >{\raggedright\arraybackslash}X@{}}
\toprule
